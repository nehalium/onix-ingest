...
BEGIN
	...
	#Update existing records, then insert new ones
		...
		UPDATE 		onix.productmasterfile pmf,
					onix.productmasterfile_stage pmfs
		...
		SET
					...
					pmf.Format = 
						CASE WHEN EXISTS (SELECT id FROM onix.productformat WHERE id = pmfs.Format)
						THEN pmfs.Format
						ELSE '--'
						END,
					...
					pmf.Availability = 
						CASE WHEN EXISTS (SELECT id FROM AvailabilityStatusCode WHERE id = pmfs.Availability)
						THEN pmfs.Availability
						ELSE '--'
						END,
		...
		WHERE 
					pmf.ISBN13 = pmfs.ISBN13
			OR		pmf.ISBN10 = pmfs.ISBN10
			OR 		pmf.EAN13 = pmfs.EAN13
			OR 		pmf.ISMN = pmfs.ISMN
			OR 		pmf.PPN = pmfs.PPN;
